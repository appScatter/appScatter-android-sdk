...
package com.appscatter.iab.utils;

import com.appscatter.iab.utils.exception.WrongThreadException;
...
public final class ASChecks {
    ...
    /**
     * Checks is the current thread main or not. Throws {@link com.appscatter.iab.utils.exception.WrongThreadException}
     * if the check failed.
     *
    ...
    public static void checkThread(final boolean mainThreadExpected) {
        final boolean isMainThread = ASUtils.isMainThread();
    ...
    public static void checkReceiver(@NonNull final Context context,
        ...
        //TODO: Check to see if PackageManager.GET_RESOLVED_FILTER gets the same effect as PackageManager.GET_INTENT_FILTER, as the later is not supported
        final List<ResolveInfo> receivers = packageManager
                .queryBroadcastReceivers(broadcastIntent, PackageManager.GET_RESOLVED_FILTER);
        if (receivers == null || receivers.isEmpty()) {
            throw new IllegalStateException("No receivers for intent "
                    + ASUtils.toString(broadcastIntent));
        }
