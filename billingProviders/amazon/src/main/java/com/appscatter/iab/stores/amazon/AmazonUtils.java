...
public final class AmazonUtils {
     ...
     */
    @Status
    public static int handleFailure(@NonNull final Context context) {
        // Unfortunately Amazon doesn't report a reason for error
        if (!PurchasingService.IS_SANDBOX_MODE && !ASUtils.isConnected(context)) {
    ...
    public static SkuDetails convertSkuDetails(@NonNull final Product product) {
        ...
        try {
            ...
            ASLog.e("Failed to set original JSON for SkuDetails.", exception);
            return null;
        }
        ...
        switch (productType) {
            case CONSUMABLE:
                builder.setType(CONSUMABLE);
                break;
            case ENTITLED:
                builder.setType(SkuType.ENTITLEMENT);
                break;
            case SUBSCRIPTION:
                builder.setType(SUBSCRIPTION);
                break;
            default:
                ASLog.e("Unknown Amazon product type: " + productType);
                return null;
        }
    ...
    public static Purchase convertPurchase(@NonNull final Receipt receipt,
        ...
        try {
            ...
            ASLog.e("Can't add Amazon userdata to original JSON: " + userData, e);
            return null;
        }
